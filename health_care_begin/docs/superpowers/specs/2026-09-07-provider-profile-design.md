...
> ⚠️ **2026-09-07 落地后修订**:主营分类 categoryId 改为**只读不可改**(用户拍板——入驻归属信息,同登录手机号口径,分类决定商家归类归属,如需调整联系平台)。编辑白名单收窄为 providerName / legalPerson / intro / address 四字段;后端不再接收/校验 categoryId(注入的 serviceCategoryMapper 与「主营服务分类不存在」校验一并删除),前端弹窗分类 el-select 置 disabled + 提示「入驻分类,暂不可修改」,保存请求不再携带。正文各节仍为修订前定稿,与本注记冲突处以本注记为准。
